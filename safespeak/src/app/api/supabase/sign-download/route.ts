...
export async function POST(request: Request) {
  ...
  const { path, expiresIn, download } = body as {
    path?: unknown;
    expiresIn?: unknown;
    download?: unknown;
  };
  ...
  const shouldDownload = typeof download === "boolean" ? download : false;

  try {
    const admin = getSupabaseAdmin();
    const { data, error } = await admin.storage
      .from(BUCKET_ID)
      .createSignedUrl(safePath, effectiveExpiresIn, { download: shouldDownload });

    if (error) {
      console.error("Supabase signed download URL error:", error.message);
      return NextResponse.json({ error: "Failed to create signed download URL." }, { status: 500 });
    }

    if (!data?.signedUrl) {
      console.error("Supabase signed download URL returned empty data.");
    ...
    console.error("Supabase signed download route failure:", message);
    return NextResponse.json(
