...
class InfiniteGridMenu {
  ...
  private initTexture(): void {
    ...
    // Create colored cards instead of loading images
    const colors = ['#5227ff', '#ff2752', '#27ff52', '#52ffff', '#ff5227'];
    for (let i = 0; i < itemCount; i++) {
      ...
      const text = this.items[i]?.name || `NGO ${i + 1}`;
      ctx.fillText(text.substring(0, 15), x + cellSize / 2, y + cellSize / 2 - 20);
      
      ctx.font = '24px Arial';
      ctx.fillText(this.items[i]?.description || 'Help Organization', x + cellSize / 2, y + cellSize / 2 + 20);
    }
...
const defaultItems: MenuItem[] = [
  {
    name: 'Help Organization',
    description: 'NGO providing support'
  }
];
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <div className={`ngo-info-panel ${isMoving ? 'inactive' : 'active'}`}>
          <h2 className="ngo-name">{activeItem.name}</h2>
          ...
          <div className="ngo-details">
            ...
            {activeItem.distanceKm !== undefined && (
              <div className="ngo-distance">
                📍 {activeItem.distanceKm.toFixed(1)} km away
              </div>
            )}
            
            {activeItem.verified && (
              <div className="ngo-verified">
