...
interface ThreeRefs {
  ...
  composer: any; // Using any for EffectComposer due to type issues
  stars: THREE.Points[];
...
export const Component = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (refs.camera && (refs as any).targetCameraX !== undefined) {
        const smoothingFactor = 0.05; // Lower = smoother but slower
        
        // Calculate smooth position with easing
        smoothCameraPos.current.x += ((refs as any).targetCameraX - smoothCameraPos.current.x) * smoothingFactor;
        smoothCameraPos.current.y += ((refs as any).targetCameraY - smoothCameraPos.current.y) * smoothingFactor;
        smoothCameraPos.current.z += ((refs as any).targetCameraZ - smoothCameraPos.current.z) * smoothingFactor;
        ...
        (refs.camera as THREE.PerspectiveCamera).position.x = smoothCameraPos.current.x + floatX;
        (refs.camera as THREE.PerspectiveCamera).position.y = smoothCameraPos.current.y + floatY;
        (refs.camera as THREE.PerspectiveCamera).position.z = smoothCameraPos.current.z;
        (refs.camera as THREE.PerspectiveCamera).lookAt(0, 10, -600);
      }
      ...
      if (refs.composer) {
        (refs.composer as EffectComposer).render();
      }
    };
    ...
    const handleResize = () => {
      ...
      if (refs.camera && refs.renderer && refs.composer) {
        (refs.camera as THREE.PerspectiveCamera).aspect = window.innerWidth / window.innerHeight;
        (refs.camera as THREE.PerspectiveCamera).updateProjectionMatrix();
        refs.renderer.setSize(window.innerWidth, window.innerHeight);
        (refs.composer as EffectComposer).setSize(window.innerWidth, window.innerHeight);
      }
    };
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      (refs as any).targetCameraX = currentPos.x + (nextPos.x - currentPos.x) * sectionProgress;
      (refs as any).targetCameraY = currentPos.y + (nextPos.y - currentPos.y) * sectionProgress;
      (refs as any).targetCameraZ = currentPos.z + (nextPos.z - currentPos.z) * sectionProgress;
      // Smooth parallax for mountains
      refs.mountains.forEach((mountain: any, i: number) => {
        const speed = 1 + i * 0.9;
        const targetZ = mountain.userData.baseZ + scrollY * speed * 0.5;
        (refs.nebula as THREE.Mesh).position.z = (targetZ + progress * speed * 0.01) - 100;
    ...
    return (
      <div className="h-screen w-full flex items-center justify-center bg-black text-white">
        <div className="text-center p-8 bg-black/50 rounded-lg">
          <h2 className="text-2xl font-bold mb-2">Error Loading Background</h2>
    ...
    <div ref={containerRef} className="hero-container cosmos-style">
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/80 z-50">
          <div className="animate-pulse text-white">Loading 3D background...</div>
        </div>
      )}
